...
bool StreamingModel::load_model_() {
  ...
  // Probe for the actual required tensor arena size only if explicitly enabled via manifest flag
  if (this->probe_arena_ && !this->tensor_arena_size_probed_) {
    size_t probed_size = this->probe_arena_size_();
    if (probed_size > 0) {
      ESP_LOGI(TAG, "MWW: probed arena size = %zu bytes", probed_size);
...
bool StreamingModel::perform_streaming_inference(const int8_t features[PREPROCESSOR_FEATURE_SIZE]) {
  ...
  if (this->loaded_) {
    ...
    if (this->current_stride_step_ >= stride) {
      uint64_t t0 = this->log_timing_ ? esp_timer_get_time() : 0;
...
bool StreamingModel::register_streaming_ops_(tflite::MicroMutableOpResolver<21> &op_resolver) {
  ...
  if (op_resolver.AddSplitV() != kTfLiteOk)
    return false;
  // BC-ResNet int8-Pipeline emittiert 26 DEQUANTIZE-Calls für PTQ-FIFO-State-Variables;
  // ohne diesen Op crasht AllocateTensors mit Op-Missing-Error.
